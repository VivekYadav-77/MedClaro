...
export function EmergencyCard({
  ...
  return (
    <Card className="space-y-4 border-red-200 p-5">
      <div>
        <p className="text-xs font-semibold uppercase tracking-wider text-red-600">Emergency Card</p>
        <h2 className="mt-1 font-display text-lg font-bold text-slate-900">{user.name}</h2>
      </div>
      ...
      {qrDataUrl ? <img src={qrDataUrl} alt="Emergency medical QR code" className="mx-auto h-36 w-36 rounded-lg border border-slate-200 bg-white p-2" /> : null}
      <Button variant={broadcasting ? "danger" : "outline"} className="w-full gap-2" onClick={broadcast}>
