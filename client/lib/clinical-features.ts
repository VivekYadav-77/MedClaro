...
export function buildClinicalFeatureCards(reports: Report[], circleCount: number): ClinicalFeatureCard[] {
  ...
  return [
    ...
    },
    {
      ...
      description: hasReports ? "Doctor export uses backend EHR rows from saved report analysis with LOINC hints." : "Upload a report to prepare doctor export rows.",
    ...
    },
    {
      ...
      shortTitle: "ICE Card",
      description: hasReports || hasPrescription ? "ICE summary uses visible report flags and saved prescription medicines." : "Upload reports or prescriptions to enrich the ICE summary.",
      status: "live",
    ...
    },
    {
      ...
      description: hasPrescription
        ? "Manage saved prescriptions, select related analyzed reports, then run confidence-aware analysis."
        : "Upload a prescription, then select related analyzed reports for better understanding.",
      status: hasPrescription ? "live" : "no_data",
      route: hasPrescription ? "/reports/medications" : "/reports/upload?type=prescription",
      actionLabel: hasPrescription ? "Manage context" : "Start flow",
      category: "clinical",
    },
    {
      id: "lab_variance",
    ...
    },
    {
      ...
      description: hasAbnormal ? "Saved abnormal markers seed coaching pathways with weekly habits and marker goals." : "Upload qualifying markers such as HbA1c, liver, or BP records.",
      status: hasAbnormal ? "live" : "no_data",
      route: "/pathways",
