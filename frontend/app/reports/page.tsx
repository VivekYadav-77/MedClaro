...
export default function ReportsPage() {
  ...
  async function loadAnalyses() {
    setStatus("loading");
    try {
      const response = await fetch(`${API_URL}/report-analyses/`, {
        headers: token ? { Authorization: `Token ${token}` } : {}
      });
      if (!response.ok) {
        throw new Error("Could not load report analyses");
      }
      const data = (await response.json()) as ReportAnalysis[];
      setAnalyses(data);
  ...
  async function runAnalysis(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error("Could not analyze report");
      }
      const analysis = (await response.json()) as ReportAnalysis;
            ...
            <div className="mt-5 space-y-4">
              ...
              {status === "error" ? (
                <p className="flex items-center gap-2 text-sm font-medium text-claro-rose">
                  ...
                  Request failed. Check token and lab report document ID.
                </p>
              ) : null}
            </div>
          </form>
