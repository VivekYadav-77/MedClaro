...
export default function PrescriptionsPage() {
  ...
  async function loadAnalyses() {
    setStatus("loading");
    try {
      const response = await fetch(`${API_URL}/prescriptions/analyses/`, {
        headers: token ? { Authorization: `Token ${token}` } : {}
      });
      if (!response.ok) {
        throw new Error("Could not load prescription analyses");
      }
      const data = (await response.json()) as PrescriptionAnalysis[];
      setAnalyses(data);
  ...
  async function runAnalysis(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const analysis = (await response.json()) as PrescriptionAnalysis;
            ...
            <div className="mt-5 space-y-4">
              ...
              {status === "error" ? (
                <p className="flex items-center gap-2 text-sm font-medium text-claro-rose">
                  ...
                  Request failed. Check token and prescription document ID.
                </p>
              ) : null}
            </div>
          </form>
