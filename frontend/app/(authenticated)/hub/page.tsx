...
export default function HubPage() {
  ...
  async function loadDashboard() {
    setStatus("loading");
    try {
      setDashboard(await apiGet<Dashboard>("/health-hub/dashboard/", token));
      setStatus("loaded");
    } catch {
      setStatus("error");
    }
            ...
            <div className="mt-5 space-y-4">
              ...
              {status === "error" ? (
                <p className="flex items-center gap-2 text-sm font-medium text-claro-rose">
                  ...
                  Request failed. Check your session and available health data.
                </p>
              ) : null}
            </div>
          </section>
            ...
            <div className="mt-4 max-h-[420px] space-y-3 overflow-y-auto">
              {!conversation ? (
                <p className="rounded-md border border-dashed border-slate-300 p-4 text-sm text-slate-600">
                  Ask a question after signing in. The assistant uses
